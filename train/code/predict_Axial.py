...
logger.info('do_train: {}, n_iter: {}, batch_size: {}, predictor: {},  saving to: {}, Splits 1 2 3 5 6 7 8  255-scaling normalized pos_weight of 2'.format(do_train, n_iters, batch_size, predictor, save_dir))
...
if do_train:
    while (iter != n_iters) and (not early_stop):
        ...
        train_losses.append(current_loss/len(df_train))
        if iter % print_every == 0:
            ...
            val_losses.append(current_loss/len(df_val))
...
with torch.no_grad():
    ...
    for j, data in enumerate(train_loader):
        ...
        st = ed
        sys.stdout.flush()
pkl.dump({'labels': y_true, 'preds': y_pred, 'probs': y_prob}, open(save_dir+'/train_predictions.pkl', 'wb'))
...
logger.info("TRAIN - {}".format(test_results_str))
...
with torch.no_grad():
    ...
    for j, data in enumerate(val_loader):
        ...
        sys.stdout.flush()
print(set(y_true), set(y_pred), len(y_true), len(y_pred), len(y_prob))
pkl.dump({'labels': y_true, 'preds': y_pred, 'probs': y_prob}, open(save_dir+'/val_predictions.pkl', 'wb'))
val_results = utils.eval_dict(y=y_true, 
                                    y_pred=y_pred, 
                                    y_prob=y_prob, 
                                    average='macro',
...
if do_train:
    ...
    plt.grid()
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, "graph.jpg"))
